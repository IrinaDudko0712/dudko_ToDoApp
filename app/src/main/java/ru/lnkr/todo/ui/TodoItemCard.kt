...
fun TodoItemCard(
    ...
    val cardColor = if (item.isCompleted) {
        AppTheme.colors.colorGreen.copy(alpha = 0.3f)
    } else {
        when (item.importance) {
            Importance.HIGH -> AppTheme.colors.colorRed.copy(alpha = 0.3f)
            Importance.NONE -> AppTheme.colors.colorBlue.copy(alpha = 0.3f)
            Importance.LOW -> AppTheme.colors.backSecondary
        }
    }
